 ...
 * Run nsp -> clean build -> update tsconfig.json -> show tslint errors -> run npm.
 * @param done - done callback function.
 */
gulp.task('build-scripts',function(done) {
 // runSequence('nsp', 'clean-build', 'tsconfig-update', ['tslint'], 'npm', done);
  runSequence('clean-build', 'tsconfig-update', ['tslint'], 'npm', done);
});
